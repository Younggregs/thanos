...
export default class MediaUploadScreen extends React.Component {
  ...
  componentDidMount(){
    ...
    if(navigation.getParam("status", 0)){
      this.setState({ upload_to_shop: true })
    }else{}

  }

  upload(){
    if(this.state.upload_to_shop){
      this.submiteShop()
    }else{
      this.submit()
    }
 ...
 async submiteShop(){
    ...
    const { navigation } = this.props;
    const account_id = navigation.getParam('account_i', 2)
    const category = navigation.getParam('category_id', 1)
    const subcategory = navigation.getParam('subcategory_id', 1)
    ...
    const starting_price = navigation.getParam('starting_price', 'starting price')
    ...
    try {
      const res = await fetch('https://www.iwansell.com/api/new_eshop_product/' + account_id + '/', {
...
async submit(){
  ...
  try {
    const res = await fetch('https://www.iwansell.com/api/newproduct/' + account_id + '/', {
     body : formData,
     method: 'POST',
     header: {
      'content-type': 'multipart/form-data',
      ...
      <View style={styles.bigContainer}>
          ...
          <HeaderText style={{ color: 'white', flex: 9 }}>Upload Product Images</HeaderText>
            ...
            {this.state.message.code && (
              this.props.navigation.navigate('ProductDisplay', {product_id: this.state.message.code})
            )}
